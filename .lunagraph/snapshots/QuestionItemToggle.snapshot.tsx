...
export default function QuestionItemToggleSnapshot({
  __onRender,
  question,
  similarity,
  isAllowed,
  ...
  onQuestionChange?: ((question: string) => void) | undefined
  onSimilarityChange?: ((similarity: SimilarityMatch) => void) | undefined
} & { __onRender?: (element: React.ReactElement) => void }) {
  console.log('[QuestionItemToggleSnapshot] Rendered with props:', { question, similarity, isAllowed, isEditMode, isShowToggle, onEditModeChange, onQuestionChange, onSimilarityChange })
  ...
  // Ref to capture which element is returned (for conditional rendering)
  const __elementRef = React.useRef<React.ReactElement | null>(null)

  // Create stable key from all props for dependency tracking
  const __propsKey = JSON.stringify({ question, similarity, isAllowed, isEditMode, isShowToggle, onEditModeChange, onQuestionChange, onSimilarityChange })

  // Pass element tree to parent via callback when props change
  React.useLayoutEffect(() => {
    if (__onRender && __elementRef.current) {
      __onRender(__elementRef.current)
    }
    ...
    const __el = (
      <div className="flex flex-col gap-2">
        <div className="flex flex-row gap-2 items-start">
          <Card className="w-full min-w-[600px] shadow-none hover:shadow-sm p-6">
            <CardContent className="h-fit flex flex-col gap-6">
              <div className="flex flex-col gap-1.5">
                ...
                <Input placeholder="Do I need to submit a preclearance request before doing a trade?" defaultValue={question} onChange={e => onQuestionChange?.(e.target.value)} className="h-fit px-3.5 py-2.5" />
              </div>
              <div className="flex flex-col gap-1.5">
                <Label className="w-fit h-fit">How similar must the question be?</Label>
                <ToggleGroup type="single" variant="outline" defaultValue={similarity} value={similarity} onValueChange={value => value && onSimilarityChange?.(value as SimilarityMatch)}>
          ...
          {isShowToggle && <Tabs defaultValue={isAllowed ? "yes" : "no"} className="w-fit h-fit mt-3.5">
  ...
  const __el = (
    <div className="flex flex-row gap-2 items-center">
      ...
      {isShowToggle && <Tabs defaultValue={isAllowed ? "yes" : "no"} className="w-fit h-fit">
          <TabsList className="h-full">
            <Tooltip>
              <TooltipTrigger className="w-fit h-fit">
                <TabsTrigger value="yes" className="w-fit text-primary">
